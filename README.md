...
cached result is reused when the project, module, source contents, compiler
context, and Swift toolchain match. Source file paths in compiler arguments are
represented by their content hashes, so moving an unchanged file avoids
re-indexing while the cached declaration is rebased to its new path. Set
